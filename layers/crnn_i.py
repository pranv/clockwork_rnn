'''
	...
	Same as CRNN, except that slower modules do not take inputs.
	Only the fastest module takes in inputs
...
class CRNN_I(Layer):
	...
	def backward(self, dY):
		...
		for t in reversed(xrange(T)):
			...
			dWi += np.dot(dH1[:nstates], input.T)
			dX[t] = np.dot(self.Wi.T, dH1[:nstates])[:-1]
