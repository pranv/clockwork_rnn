...
def recurrent_mask(nclocks, nstates):
	...
	for c in range(1, nclocks - 1):
		one_blocks = np.ones((nstates, nstates * (c)))
		zero_blocks = np.zeros((nstates, nstates * (nclocks - c)))	
		matrix.append(np.concatenate([one_blocks, zero_blocks], axis=1))
	matrix.append(np.ones((nstates, nstates * (nclocks))))
...
class CRNN_HTM(Layer):
	def __init__(self, dinput, nstates, doutput, clock_periods, full_recurrence=False, learn_state=False, first_layer=False):
		...
		''' 
		_clocks = [1]
		_clocks.extend(clock_periods)
		_clocks.append(1)
		clock_periods = _clocks
	...
	def forward(self, X):
		...
		if self.H_last is not None:
			...
			H_prev = np.zeros((nclocks * states, B))
